...
__global__ void fa_split_gqa_kernel(
    ...
    // consecutive dims {2*lane + pr*PSTRIDE, +1}. Reading the KV tile as bf16x2 (4B)
    // instead of scalar bf16 makes the hot inner-loop smem loads conflict-free — 32
    // lanes touch 32 distinct 4B banks — and halves the load count. (The prior scalar
    // s_k[lane + e*32] read had 32 lanes hit 16 banks: a 2-way bank conflict.)
    ...
    for (int t0 = start; t0 < end; t0 += TILE) {
        ...
        __syncthreads();
        // Vectorized load: bf16x2 from global straight into bf16x2 smem slots.
